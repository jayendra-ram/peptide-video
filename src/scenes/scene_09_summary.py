...
from src.manim.equation_helpers import condensation_short
...
class SummaryScene(PeptideSceneBase):
    ...
    def construct(self) -> None:
        ...
        bullets_data = [
            r"$\bullet$ HOMO/LUMO overlap drives nucleophilic attack",
            r"$\bullet$ $\Delta G^{\ddagger}$ requires catalysis (ribosome)",
            r"$\bullet$ Amide resonance $\rightarrow$ planar backbone",
        ]

        bullet_mobjects = []
        for i, text in enumerate(bullets_data):
            if text.startswith(r"$\bullet$"):
                # Use MathTex for bullet points with LaTeX content
                bullet = MathTex(
                    text,
                    font_size=22,
                    color=WHITE,
                ...
                bullet = Text(text, font_size=20, color=WHITE)
        ...
        final_eq = MathTex(
            r"2\;\text{Gly}",
            r"\rightarrow",
            r"\text{Gly-Gly} + \text{H}_2\text{O}",
            r",\quad \Delta G^{\circ} \approx +10\;\text{kJ/mol}",
            font_size=26,
